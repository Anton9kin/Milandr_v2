...
//		getScene().stopSetupProcess();
	...
	protected boolean checkPairForHide(String pairName) {
		...
		if (getScene().isProdMode()) {
			if (Device.testPairNames().contains(pairName)) return false;
			if (Device.prodPairNames().contains(pairName)) return false;
		}
		return !Device.showPairNames().contains(pairName);
	}
	...
	protected void makeUI(Device.EPairNames pair, String group) {
		...
		if (isPropsGrid) {
			if (McuBlockProperty.USE_HBOX) {
				makeColumnConstraints((GridPane) propsPane, 99, 1);
			} else makeColumnConstraints((GridPane) propsPane, pair.colWidth(), 100 - pair.colWidth());
		}
		...
		reCreatePinProps(pair, group, propsPane, pairCnt);
	}

	protected void reCreatePinProps(Device.EPairNames pair, String group, Pane propsPane, int pairCnt) {
	...
	protected McuBlockProperty getNamedProp(List<McuBlockProperty> props, String name, McuBlockProperty.PropAction action) {
		for(McuBlockProperty prop: props) {
			if (prop.getName().equals(name)) {
				if (action != null) action.exec(prop);
				return prop;
			}
		}
		return null;
